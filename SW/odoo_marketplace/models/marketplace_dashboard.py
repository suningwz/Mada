...
class marketplace_dashboard(models.Model):
        ...
        if self.state == 'order':
            if self.is_seller:
                ...
                obj = self.env['sale.order.line'].search(
                    [('marketplace_seller_id', '=',user_obj.partner_id.id), ('marketplace_state', '=', 'approved')])
            else:
                obj = self.env['sale.order.line'].search(
                    [('marketplace_seller_id', '!=', False), ('marketplace_state', '=', 'approved')])
            self.count_product_approved = len(obj)
    ...
    def _get_pending_count(self):
        ...
        if self.state == 'order':
            ...
            if self.is_seller:
                obj = self.env['sale.order.line'].search(
                    [('state','not in',('draft','sent')), ('marketplace_seller_id', '=',user_obj.partner_id.id), ('marketplace_state', '=', 'new')])
            else:
                obj = self.env['sale.order.line'].search(
                    [('state','not in',('draft','sent')), ('marketplace_seller_id', '!=', False), ('marketplace_state', '=', 'new')])
            self.count_product_pending = len(obj)
        if self.state == 'payment':
            obj = self.env['seller.payment'].search(
    ...
    def _get_rejected_count(self):
        ...
        if self.state == 'order':
            ...
            if self.is_seller:
                obj = self.env['sale.order.line'].search(
                    [('marketplace_seller_id', '=',user_obj.partner_id.id), ('marketplace_state', '=', 'shipped')])
            else:
                obj = self.env['sale.order.line'].search(
                    [('marketplace_seller_id', '!=', False), ('marketplace_state', '=', 'shipped')])
            self.count_product_rejected = len(obj)
