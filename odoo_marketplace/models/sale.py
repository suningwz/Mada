...
class SaleOrder(models.Model):
    ...
    def action_confirm(self):
        ...
        if config_setting_obj.get("enable_notify_seller_on_new_order", False) and config_setting_obj.get("notify_seller_on_new_order_m_tmpl_id", False):
            template_obj = self.env['mail.template'].browse(config_setting_obj.get("notify_seller_on_new_order_m_tmpl_id"))
            for order in self:
                seller_objs = order.order_line.mapped('marketplace_seller_id')
                for seller in seller_objs:
                    try:
                        template_obj.with_context(seller=seller).send_mail(order.id, True)
...
class SaleOrderLine(models.Model):
    ...
    def action_view_sol_seller_payment(self):
        self.ensure_one()
        seller_payment_objs = self.order_id.sudo().invoice_ids.mapped('seller_payment_ids').filtered(lambda sp: sp.seller_id.id == self.marketplace_seller_id.id)
        action = self.env.ref('odoo_marketplace.wk_seller_payment_action').read()[0]
    ...
    def _prepare_procurement_values(self, group_id=False):
        ...
        if marketplace_seller_obj:
            seller_warehouse_obj = marketplace_seller_obj.warehouse_id
            if seller_warehouse_obj:
