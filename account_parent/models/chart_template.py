...
class AccountChartTemplate(models.Model):
    ...
    def generate_account(self, tax_template_ref, acc_template_ref, code_digits, company):
        ...
        account_template_account_dict = super(AccountChartTemplate, self).generate_account(
        ...
        for account_template_id, account_id in account_template_account_dict.items():
            account_template_obj = account_tmpl_pool.browse(
                account_template_id)
#             account_template_obj.update_account_with_parent(account_template_account_dict[account_template_obj.id],company)
            ...
            account_pool.browse(account_id).write(
    ...
    def update_generated_account(self, tax_template_ref=[], code_digits=1, company=False, importing_parent=False):
        ...
        for account_template in acc_templates:
            ...
            if account_template.user_type_id.id == view_liquidity_type.id:
                ...
                new_code = code_acc
            new_account = self.env['account.account'].with_context({'show_parent_account': True}).search(
            ...
            if not new_account:
                ...
                new_account_id = self.create_record_with_xmlid(
                    company, account_template, 'account.account', vals)
                new_account = self.env['account.account'].with_context(
                    {'show_parent_account': True}).browse(new_account_id)
...
#             account_template.update_account_with_parent(new_account.id,company)
        ...
        if company.transfer_account_code_prefix:
            if code_account_dict.get(company.transfer_account_code_prefix, False):
                parent_account_id = code_account_dict.get(
                    company.transfer_account_code_prefix, False)
            else:
                parent_account_id = account_obj.with_context({'show_parent_account': True}).search([
...
#         all_acc_templates = acc_templates.with_context({'company_id':company.id})
        ...
        for account_template in acc_templates:
            ...
            template_xml_obj = ir_model_data.search(
                [('model', '=', account_template._name), ('res_id', '=', account_template.id)])
            account_xml_id = "%s.%s_%s" % (
                template_xml_obj.module, company.id, template_xml_obj.name)
            account = self.env.ref(account_xml_id, raise_if_not_found=False)
            parent_template_xml_obj = ir_model_data.search(
                [('model', '=', account_template._name), ('res_id', '=', account_template.parent_id.id)])
            parent_account_xml_id = "%s.%s_%s" % (
                parent_template_xml_obj.module, company.id, parent_template_xml_obj.name)
            parent_account = self.env.ref(
                parent_account_xml_id, raise_if_not_found=False)
            account.write({'parent_id': parent_account.id})
        return True

    def load_for_current_company(self, sale_tax_rate, purchase_tax_rate):
        res = super(AccountChartTemplate, self).load_for_current_company(
            sale_tax_rate=sale_tax_rate, purchase_tax_rate=purchase_tax_rate)
        self.update_generated_account(
            {}, self.code_digits, self.env.user.company_id)
        return res
