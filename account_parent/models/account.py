...
class AccountAccount(models.Model):
    ...
    def _move_domain_get(self, domain=None):
        ...
        if context.get('reconcile_date'):
            domain += ['|', ('reconciled', '=', False), '|', ('matched_debit_ids.create_date', '>',
                                                              context['reconcile_date']), ('matched_credit_ids.create_date', '>', context['reconcile_date'])]
...
#     parent_right = fields.Integer('Right Parent', index=1)
    ...
    def default_get(self, fields):
        res = super(AccountAccount, self).default_get(fields)
        if self.env.context.get('default_is_parent'):
            res['user_type_id'] = self.env.ref('account_parent.data_account_type_view').id
        return res
